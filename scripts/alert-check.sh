...
if [ "$curr_down" != "$prev_down" ]; then
  if [ ${#down[@]} -eq 0 ]; then
    # Recovery
    curl -s -X POST "$NTFY"       -H "Title: ✅ All Services Recovered"       -H "Priority: default"       -H "Tags: white_check_mark,server"       -d "Previously down: $prev_down — all services are now responding." > /dev/null
  else
    # New outage
    msg="${down[*]}"
    n=${#down[@]}
    curl -s -X POST "$NTFY"       -H "Title: 🔴 $n Service$([ $n -gt 1 ] && echo s) Down"       -H "Priority: high"       -H "Tags: rotating_light,server"       -d "$msg" > /dev/null
  fi
fi
